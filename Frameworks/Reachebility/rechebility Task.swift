//
//  rechebility Task.swift
//  internetCheckker
//
//  Created by User on 04/11/1938 Saka.
//  Copyright © 1938 Saka KangarooSoftware Pvt. All rights reserved.
//
...
class ReachabilityManager: NSObject
{
    ...
    func createInternetView(CreateView : Bool)
    {
        if let UIWindow = UIApplication.shared.keyWindow
        {
            if let RootVc = UIWindow.rootViewController
            {
                if let mainVC : UIView = RootVc.view
                {
                    if CreateView == true
                    {
                        ...
                        myView.tag = 786
                        myView.image = #imageLiteral(resourceName: "Internet Passenger copy")
                        myView.contentMode = .scaleAspectFit
                        
                        mainVC.addSubview(myView)
                        
                        myView.translatesAutoresizingMaskIntoConstraints = false
                        
                        mainVC.addConstraint(NSLayoutConstraint(item: myView, attribute: .centerX, relatedBy: .equal, toItem: mainVC, attribute: .centerX, multiplier: 1.0, constant: 0))
                        
                        mainVC.addConstraint(NSLayoutConstraint(item: myView, attribute: .centerY, relatedBy: .equal, toItem: mainVC, attribute: .centerY, multiplier: 1.0, constant: 0))
                        
                        mainVC.addConstraint(NSLayoutConstraint(item: myView, attribute: .height, relatedBy: .equal, toItem: mainVC, attribute: .height, multiplier: 0.4, constant: 0))
                        
                        mainVC.addConstraint(NSLayoutConstraint(item: myView, attribute: .width, relatedBy: .equal, toItem: mainVC, attribute: .width, multiplier: 0.8, constant: 0))
                    }
                    else
                    {
                        if let removeInternet = mainVC.viewWithTag(786)
                        {
                            removeInternet.removeFromSuperview()
                        }
                        if let removeInterenetBlue : UIVisualEffectView = mainVC.viewWithTag(787) as? UIVisualEffectView
                        {
                            removeInterenetBlue.removeFromSuperview()
                        }
                    }
