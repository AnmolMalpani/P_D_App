...
public typealias dropDownIndex = Int
public typealias Closure = () -> Void
public typealias SelectionClosure = (dropDownIndex, String) -> Void
public typealias ConfigurationClosure = (dropDownIndex, String) -> String
public typealias CellConfigurationClosure = (dropDownIndex, String, DropDownCell) -> Void
...
public final class DropDown: UIView {
	...
	*/
	...
	/// The dropDownIndex of the row after its seleciton.
	fileprivate var selectedRowIndex: dropDownIndex?
...
private extension DropDown {
	...
	func setupUI() {
        ...
        tableView.separatorInset = UIEdgeInsets(top: 0.1, left: 0, bottom: 0.1, right: 0)
...
extension DropDown {
	...
	fileprivate func fittingWidth() -> CGFloat {
		...
		for dropDownIndex in 0..<dataSource.count {
			configureCell(templateCell, at: dropDownIndex)
...
extension DropDown {
	...
	*/
	...
	/// (Pre)selects a row at a certain dropDownIndex.
	public func selectRow(at dropDownIndex: dropDownIndex?) {
		if let dropDownIndex = dropDownIndex {
			tableView.selectRow(
				at: IndexPath(row: dropDownIndex, section: 0),
				animated: false,
		...
		selectedRowIndex = dropDownIndex
	}

	public func deselectRow(at dropDownIndex: dropDownIndex?) {
		selectedRowIndex = nil

		guard let dropDownIndex = dropDownIndex
			, dropDownIndex >= 0
			else { return }

		tableView.deselectRow(at: IndexPath(row: dropDownIndex, section: 0), animated: true)
	}

	/// Returns the dropDownIndex of the selected row.
	public var indexForSelectedRow: dropDownIndex? {
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		let dropDownIndex = (indexPath as NSIndexPath).row

		configureCell(cell, at: dropDownIndex)

		return cell
	}
	
	fileprivate func configureCell(_ cell: DropDownCell, at dropDownIndex: Int) {
		if dropDownIndex >= 0 && dropDownIndex < localizationKeysDataSource.count {
			cell.accessibilityIdentifier = localizationKeysDataSource[dropDownIndex]
		}
        ...
        {
			cell.optionLabel.text = " " + cellConfiguration(dropDownIndex, dataSource[dropDownIndex])
		}
        else
        {
			cell.optionLabel.text = " " + dataSource[dropDownIndex]
		}
		
		customCellConfiguration?(dropDownIndex, dataSource[dropDownIndex], cell)
	}
