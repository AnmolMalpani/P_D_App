...
class Weeks: UITableViewController {
...
//                        self.Spinner(Task: 2, tag: 98714)
            ...
            case .failure(_):
                print("Error in getting All Weeks")
            }
        }
    }
    ...
    {
        let cell = tableView.dequeueReusableCell(withIdentifier: "weeksCell", for: indexPath) as! weeksCell
        
        cell.textAllWeeks[0].text = allData[indexPath.row]["date"]!
        
        let price = String(describing: Double(allData[indexPath.row]["price"]!)!.roundTo(places: 2))
        
        cell.textAllWeeks[1].text = "$" + " " + price
